...
export class FinanceScheduler {
  ...
  async sendReimbursementReminder() {
    ...
    this.logger.log(
      "Checking for reimbursement requests created 4 days ago...",
    );

    const fourDaysAgo = new Date(Date.now() - 4 * 24 * 60 * 60 * 1000);

    // Find all reimbursement requests created 4 days ago
    const datedReimbursementRequests = await this.financeRepo
      .findAll()
      .byHackathon()
    ...
    await Finance.query().whereIn("id", ids).patch({ reminderSent: true });
    this.logger.log(
      `Found ${datedReimbursementRequests.length} dated reimbursement requests`,
    );
  ...
  private async sendReimbursementReminderEmail(
    datedReimbursementRequests: Finance[],
  ) {
